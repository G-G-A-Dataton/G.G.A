...
print(f"terms : {len(terms):,} satır")
print(f"items : {len(items):,} satır")
print(f"pairs : {len(pairs):,} satır")
...
print()
print(f"Kaç farklı term eğitimde kullanılmış     : {pairs['term_id'].nunique():,} / {len(terms):,}")
print(f"Kaç farklı item eğitimde kullanılmış     : {pairs['item_id'].nunique():,} / {len(items):,}")
print(f"Term başına ortalama pozitif çift        : {len(pairs) / pairs['term_id'].nunique():.1f}")
print()
